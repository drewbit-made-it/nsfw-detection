...
| `-e USE_INT8=false` | `true`: dynamic int8 quantization — faster on CPU, ~2-4x smaller model file; `false` (default): float32 |
...
services:
  ...
  nsfw-detection:
    ...
    environment:
      ...
      - MAX_IMAGE_MB=10
      - USE_INT8=false    # set true to enable int8 quantization (faster CPU inference)
    networks:
      - backend
